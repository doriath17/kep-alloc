...
TEST(SystemPageBackingTest, AllocateAndDeallocateMultipleChunks) {
    ...
    test_allocate_and_deallocate(
        backing, page_size + 1,
        "Testing allocation of page_size + 1 bytes -- page lower boundary.");
    test_allocate_and_deallocate(
        backing, page_size + (page_size >> 2),
        "Testing allocation of page_size + (page_size >> 2) bytes -- in the middle.");
    test_allocate_and_deallocate(
        backing, page_size * 2 - 1,
        "Testing allocation of page_size * 2 - 1 bytes -- page upper boundary.");
