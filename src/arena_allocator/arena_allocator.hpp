 ...
 * by simply updating an allocation offset (representing the offset from the start, at the initial state it will be 0). 
 * It is optimal to allocate memory for objects that have the same lifetime so that they can fastly be allocated
 * O(1) and they can be deallocated all at once O(1).
 * 
 * ## Use Cases
 * To understand the allocator better and how the user will use it and what he expects from it, here there are
 * three use cases. 
 * 
 * ### Game Loop: per-frame allocations
 * In game engines thousand of temporary objects (like particles) are allocated and discarded every frame.
 * Usage sequence:
 * @code{.cpp}
...
while (game_is_running) {
    ...
    auto* particles = static_cast<Particle*>(arena.allocate(sizeof(Particle) * 500, alignof(Particle)));
    auto* debug_text = static_cast<char*>(arena.allocate(128, alignof(char)));
 ...
 * Sometimes a function (like when parsing something) needs a temporary memory context to work with, but the higher level
 ...
 * they are created during parsing, used during execution/compilation, destroyed all at once when the task finishes.
 * 
 * @code{.cpp}
 ...
 * this is important to guarantee the correct working of the allocated object since the cpu and the hardware 
 ...
 * 
 * - for compound types like structs or classes the situation is quite different and it mainly depends on the members
 * alignment reqs. and specifically by the member with the largest alignment requirement. For example: 
 * @code{.cpp}
struct Example {
 ...
 * @endcode
 * Here the memeber with the largerst alignment req. is the integer with 4-bytes alignment. So the entire struct 
 * has an alignment req. of 4-bytes. 
 * Note that in this case the total size of the structure is 8byte (not 5, because you need to consider the padding)
 * and the requirement for the alignment is 4byte. This means that the alignment req. does not always correspond to the size. 
 * 
 * ## Useful for the alignment
 * - `alignof(T)` -> to obtain the alignment requirement of the type T
 * - `std::max_align_t` -> it is a POD (plain old data) type whose alignment requirement is at least as strict as 
 ...
 * It is commonly used to provide a default value for when it comes to allocate something and you dont have an
 * alignment requirement specified. So, almost like a convention, the allocator should use `alignof(std::max_align_t)`
 * to align the given type. This guarantees that any standard C++ type can be safely placed at that returned address. 
 * By the way, if you think of it, `std::max_align_t` represent the largest alignment req. a compond type member can have,
 * so at the end of day, it represents the max alignment any standard type could have (it is like maximising it).
 * 
 */
...
class ArenaAllocator {
  public:
    ArenaAllocator(backing_policy& bp, size_t init_size): m_backing(bp), m_size(init_size) {
    ...
    ~ArenaAllocator() {
        m_backing.deallocate_chunk(m_base_ptr, m_size);
    }
    ...
    [[nodiscard]] void* allocate(size_t size, size_t alignment = alignof(std::max_align_t)) {
        ...
        if (aligned_address + size > reinterpret_cast<size_t>(m_base_ptr) + m_size) {
            throw std::invalid_argument("ArenaAllocator: Out of memory. Requested size exceeds arena capacity.");
        }
     ...
     * 
     * This function is a no-op for the arena allocator since individual deallocations are not supported.
     * 
    ...
    /**
     * @brief Resets the arena allocator to its initial state, effectively deallocating all allocated memory.
     */
    void reset() {
